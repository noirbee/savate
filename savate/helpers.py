...
def build_http_headers(headers: dict[bytes, Optional[bytes]], body: bytes) -> bytes:
    ...
    return b"".join(b"%s: %s\r\n" % (key, value) for key, value in default_headers.items() if value is not None)


def event_mask_str(event_mask: int) -> str:
    masks_list = ("POLLIN", "POLLOUT", "POLLERR", "POLLHUP")
    return "|".join(mask for mask in masks_list if event_mask & getattr(looping, mask))
...
class HTTPEventHandler(BaseIOEventHandler):
    def __init__(
        ...
        self.output_buffer = buffer_event.BufferOutputHandler(sock, (response.as_bytes(),))
    ...
    def close(self) -> None:
        ...
        self.server.request_out(
            self.request_parser, self.sock, self.address, self.bytes_sent, self.connect_time, self.status
        )
    ...
    def flush(self) -> None:
        try:
            ...
            self.server.logger.info("Client queue size exceeded for %s: %s", self, exc)
            self.close()
            return None
...
class HTTPResponse:
    def __init__(
        self, status: int, reason: bytes, headers: Optional[dict[bytes, Optional[bytes]]] = None, body: bytes = b""
    ) -> None:
        self.status = status
        self.reason = reason
